...
class Net(nn.Module):
    ...
    #     self.hidden_layer1 = nn.Linear(16,128)
    #     self.hidden_layer2 = nn.Linear(128,18)
...
def model_loss_(x, u_hat, device):
    ...
    m1x = torch.index_select(x, 1, torch.tensor([8])).to(device)*6 +2
    m2x = torch.index_select(x, 1, torch.tensor([9])).to(device)*4 + 1
    L1x = torch.index_select(x, 1, torch.tensor([10])).to(device)+ 1
    L2x = torch.index_select(x, 1, torch.tensor([11])).to(device)+0.5
    I1x = torch.index_select(x, 1, torch.tensor([12])).to(device)*0.3 +0.1
    I2x = torch.index_select(x, 1, torch.tensor([13])).to(device)*0.15 + 0.05
    F1x = torch.index_select(x, 1, torch.tensor([14])).to(device)*10 + 10
    F2x = torch.index_select(x, 1, torch.tensor([15])).to(device)*10 + 10
    
    m1u = torch.index_select(u_hat, 1, torch.tensor([0])).to(device)*6 + 2
    m2u = torch.index_select(u_hat, 1, torch.tensor([1])).to(device)*4 + 1
    L1u = torch.index_select(u_hat, 1, torch.tensor([2])).to(device) + 1
    L2u = torch.index_select(u_hat, 1, torch.tensor([3])).to(device) + 0.5
    I1u = torch.index_select(u_hat, 1, torch.tensor([4])).to(device)*0.3 +  0.1
    I2u = torch.index_select(u_hat, 1, torch.tensor([5])).to(device)*0.15 + 0.05
    F1u = torch.index_select(u_hat, 1, torch.tensor([6])).to(device)*10 + 10
    F2u = torch.index_select(u_hat, 1, torch.tensor([7])).to(device)*10 + 10
...
def main():
    ...
    try:
        model_path=os.path.join(array_folder, 'deeper3_checkpoint2.pt')
    ...
    early_stop = EarlyStopper(10, 0.1, 0.000000001)
    #loss = []
    ...
    for epoch in range(iterations):
        ...
        mse_u2 = mse_cost_function(net_bc_out, pt_y_validation)
        #e, dv = model_loss(pt_x_validation, net, device)
        #mse_f = mse_cost_function(e, dv)
        valitation_loss = mse_u2 #+ mse_f
    ...
    np.save(os.path.join(array_folder, 'train_deep3.loss'), np.array(train_validation))
    np.save(os.path.join(array_folder, 'test_deep3.loss'), np.array(test_validation))


    print('saving model')
    torch.save(net.state_dict(), os.path.join(array_folder, 'deeper3.pt'))
    net = net.cpu()
    loss = loss.cpu()
    torch.save({
        'loss' : loss,
    ...
    }, os.path.join(array_folder, 'deeper3_checkpoint2.pt'))

    #print 20 predictions from test
